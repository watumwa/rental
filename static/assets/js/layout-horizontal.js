...
function getBasePath() {
  ...
  // If we're in root or index.html, no prefix needed
  if (depth <= 1 || window.location.pathname.endsWith('/index.html') || window.location.pathname.endsWith('/')) {
    return '';
  }
...
(function () {
  ...
  if (docW > 1024) {
    if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
      document.querySelector('.pc-sidebar .m-header .logo-lg').setAttribute('src', getBasePath() + 'assets/images/logo-dark.svg');
    }
    set_horizontal_menu();
    set_edge_menu();
  }
      ...
      if (list_index + 1 === pc_menu_list_new.length) {
        if (pc_sing_link.insertAdjacentHTML) {
          pc_sing_link.insertAdjacentHTML(
            ...
            '<li class="pc-item pc-hasmenu">\
                          <a href="#!" class="pc-link ">\
                              <span class="pc-micon"><i class="ph ph-grains"></i></span>\
